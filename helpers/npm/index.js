...
const Runtime = server_1.type('java.lang.Runtime');
        ...
        if (action && target) {
            if (['install', 'remove', 'update'].includes(action)) {
                if (validator.test(target)) {
                    ...
                    const process = runtime.exec(`npm install -y ${target}`, null, server_1.root.io);
                    ...
                    sender.sendMessage(`\xa7cThat target (${target}) is invalid! Example Targets: [ \xa7bprimatus\xa7c, \xa7b@grakkit/server\xa7c ]`);
                }
            }
            else {
                sender.sendMessage(`\xa7cThat action (${action}) is invalid! Valid Actions: [ \xa7binstall\xa7c, \xa7bupdate\xa7c, \xa7bremove\xa7c ]`);
            }
        }
        else {
